...
5. For non-trivial harness work, classify the task and proactively create a small core council and any needed domain specialists from `harness-experts.v2` before main execution. Use single-agent mode only for trivial inspection or tiny edits, and record the skip reason in `manifest.json`.
...
12. End only with `goal_met`, `budget_exhausted`, `policy_blocked`, `environment_blocked`, `user_decision_needed`, or `failed_with_evidence`.
...
- For the built-in expert capability library, read `references/expert-capability-library.md`.
...
If a script reports missing artifacts or invalid state, fix the harness evidence before claiming the task is complete.
